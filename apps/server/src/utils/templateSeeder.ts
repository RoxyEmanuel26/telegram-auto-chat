...
import { ParseMode, MediaType } from '@prisma/client';
...
const defaultTemplates = [
  // Category: NSFW
  {
    ...
    mediaUrl: 'https://images.unsplash.com/photo-1518895949257-7621c3c786d7?w=800&auto=format&fit=crop&q=60',
    isPublic: true,
  },
  {
...
export const seedDefaultTemplates = async () => {
  try {
    ...
    if (!user) {
      logger.info('No user found for auto-seeding templates. Skipping auto-seed.');
      return;
    }
    ...
    if (seededCount > 0) {
      logger.info(`Auto-seeded ${seededCount} default templates successfully.`);
    }
  } catch (error) {
    logger.error(`Failed to auto-seed default templates: ${error}`);
  }
};
